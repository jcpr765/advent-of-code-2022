...
const adjustTail = (
  ...
  move(direction, tail, shouldMark);
};

const knotsAreTouching = (head: Coords, tail: Coords) => {
  return Math.abs(head.x - tail.x) <= 1 && Math.abs(head.y - tail.y) <= 1;
};

const data = getTextInput("input.txt");

const startCoords: Coords = { y: 25, x: 25 };

const head: Coords = Object.assign({}, startCoords);
const tails = new Array(9);

for (let i = 0; i < tails.length; i++) {
  tails[i] = Object.assign({}, startCoords);
}
...
for (const movement of movements) {
  ...
  for (let i = 0; i < parseInt(amount); i++) {
    ...
    let parentKnot = head;
    for (let i = 0; i < tails.length; i++) {
      adjustTail(parentKnot, tails[i], i === tails.length - 1);
      parentKnot = tails[i];
    }
  }
}
